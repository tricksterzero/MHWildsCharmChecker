...
**重要な前提**: 対応・検証したのは「WQHD(2560x1440、16:9)モニタでゲーム内設定を21:9にした場合」のスクリーンショットのみ。この場合ゲームは2560x1097相当でレンダリングし、Steamが2560x1440キャンバスに上下黒帯付きで保存する。**真の21:9モニタ(例: 3440x1440等のネイティブ21:9解像度)でキャプチャした場合にどうなるか(黒帯の有無、解像度、UIスケーリングが同じ規則に従うか)は未検証・未確認**（2026-07-24時点、実機データなし）。真の21:9モニタのスクショが手に入った場合は別途検証が必要。
...
- **21:9はWQHDモニタ+ゲーム内21:9設定(黒帯付き2560x1440)の場合のみ対応済み**（詳細は上記節）。検証範囲は「装備の確認・売却」画面3枚(2026-06-15撮影)のみで、他の画面パターン(装備変更・マカ錬金等の21:9版)は未検証。**真の21:9モニタでのキャプチャは未検証**
