...
- **融合フレームの回復（`TryRecoverFusedFrame`、2026-07-21追加）**: 「装備変更」画面の2護石比較パネルで、1つ目のスロットアイコンがスロットバーの縁取り(装飾UI)とCannyの輪郭検出で1つに融合し、幅が正常範囲(`FRAME_W_RANGE`上限)を大きく超える塊として検出され、本来複数あるはずのスロットの一部が検出漏れになる問題への対応（実データで1件確認済み）。幅だけが上限〜上限×4の範囲にある塊を「融合候補」として保持し、その塊のX範囲内に既に確定した正常なフレーム(融合していない側の隣接アイコン)が含まれる場合、その左右の未検出側だけを狭い窓として切り出し再度Canny+輪郭検出をかけて回復を試みる。窓の内側境界(隣接フレーム側)は境界通り、外側境界には`FRAME_W_RANGE`上限×0.4のマージンを追加する（境界ぎりぎりで切り出すとCannyが輪郭を正しく閉じられず、本来の形状より小さい断片に分裂することを実験で確認済みのため）。窓から複数の有効候補が出た場合、窓幅が概ね1アイコン分（上限×1.5未満）なら最大面積のものを採用（同一アイコンの内側/外側境界の重複検出とみなす）、それ以上の幅なら曖昧として不採用。ローカル検証画像35枚・70領域(BOX/Detail×各画像)でのblast-radius比較（回復ON/OFF）では対象ケース1件以外に差分なしを確認済みだが、357枚での実機再検証は未実施（ローカルに保持しているのは35枚のみ）。
...
- **融合フレーム回復は357枚の全体再検証はしていない**（357枚の実機テストは2026-06〜07時点の旧ロジックに対するもの。回復ロジック追加後はローカル保持の35枚でのblast-radius比較のみ）。装備変更画面のスロット検出結果が疑わしい場合は実画像で個別確認すること。
