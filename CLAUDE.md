...
- **融合フレームの回復（`TryRecoverFusedFrame`、2026-07-21追加、同日中に2パターン目対応で拡張）**: スロットアイコンがスロットバーの縁取り(装飾UI)とCannyの輪郭検出で1つに融合し、幅が正常範囲(`FRAME_W_RANGE`上限)を大きく超える塊として検出され、本来複数あるはずのスロットの一部が検出漏れになる問題への対応。幅だけが上限〜上限×4の範囲にある塊を「融合候補」として保持し、実データで2パターン確認済み:
  ...
  - ローカル検証画像36枚・72領域(BOX/Detail×各画像)でのblast-radius比較（回復ON/OFF）では対象ケース2件以外に差分なしを確認済みだが、357枚での実機再検証は未実施（ローカルに保持しているのは36枚のみ）。
...
- **融合フレーム回復は357枚の全体再検証はしていない**（357枚の実機テストは2026-06〜07時点の旧ロジックに対するもの。回復ロジック追加後はローカル保持の36枚でのblast-radius比較のみ）。装備変更画面・単一装備詳細パネルのスロット検出結果が疑わしい場合は実画像で個別確認すること。
