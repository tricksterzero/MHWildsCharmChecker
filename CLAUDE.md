...
- **検証範囲・制約**: 検証済みは「装備変更」(2護石比較)画面のみ、17枚(2026-07-26撮影、3440x1440)。スキル名・Lv読み取りは全17枚で正解を確認(`SkillReadingPipelineTests`に代表4枚を回帰テストとして追加)。スロット検出は目視で3枚を個数・種別ベースで確認、加えて17枚全てでRARE値と武器スロット有無(RARE8=必ず武器スロットあり)の整合性を確認(矛盾ゼロ)。レベル判定の厳密な正確性はユーザーの実機動作確認待ち。単一護石詳細画面・鑑定BOX画面等、他の画面パターンでの真の21:9対応は未検証(追加スクショ入手時に最優先で検証すべき箇所)。
