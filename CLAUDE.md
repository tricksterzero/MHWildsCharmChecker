...
- **単一パネル画面(狩猟後護石入手=鑑定BOX、マカ錬金鑑定結果)は追加のコード変更なしで機能したが、box/detail領域の採用関係が画面ごとに異なる(2026-07-26追加検証)**: 「装備変更」の2護石比較とはレイアウトが大きく異なる(中央に護石アイコン画像+名前パネル、右端に単一の「装備詳細」パネルという構成)。スキル名・Lv読み取り(アンカー検出)は右端パネルの右上基準位置が共通のためそのまま機能した。スロット検出(`ReadSlots`のbox/detail採用ロジック)は画面ごとに正解パネルが違う領域に対応することを`DetectFrames`の個別カウントで確認済み:
  ...
  - 狩猟後護石入手画面4枚は全数成功。マカ錬金鑑定結果画面は4枚中3枚成功、1枚(`205B741.JPG`「未解の護石」)は「装備詳細」等のアンカーテキストがOCRで縦方向に混線して1行として認識されず失敗(3回再実行して同一結果、決定論的だが原因未特定)。
- **検証範囲・制約**: 検証済みは「装備変更」(2護石比較)17枚、狩猟後護石入手(鑑定BOX)4枚、マカ錬金鑑定結果4枚(3枚成功・1枚失敗)、合計25枚(2026-07-26撮影、3440x1440)。スキル名・Lv読み取りは25枚中24枚で正解を確認(`SkillReadingPipelineTests`に代表8枚を回帰テストとして追加)。スロット検出は目視で3枚を個数・種別ベースで確認、加えて全画像でRARE値と武器スロット有無(RARE8=必ず武器スロットあり)の整合性を確認(矛盾ゼロ)。レベル判定の厳密な正確性はユーザーの実機動作確認待ち。
