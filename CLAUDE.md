...
- **ペアリングはインデックス順ではなくY座標最近傍の貪欲マッチング**（`PairByNearestY`、2026-07-21修正）: 単純なインデックス順ペアリングだと、OCRエンジンが特定のスキル名を1つでも検出できない場合（例: 「匠」のような1文字スキル名を検出漏れするケース）、後続の全スキルのLvがずれて誤ったペアになってしまう。Y座標が近いペアから貪欲に確定させることで、検出漏れがあった行だけが名前欠落(null)として扱われ、他の行のペアリングは影響を受けない
